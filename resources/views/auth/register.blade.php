                ...
                <section class="login_content">
                    <form action="{{ route('register') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <select type="text" id="state" name="state" class="form-control" onchange="Func(this.options[this.selectedIndex].getAttribute('data-cid'))">
...
{{--                                <option data-cities="  ,ابرکوه ,اردکان ,بافق ,تفت ,خاتم ,صدوق ,طبس ,مهریز ,مِیبُد ,یزد" value="یزد">یزد</option>--}}
    ...
    <script>
        ...
        function Func(city) {
            ...
            $.ajax({
                type: 'GET',
                url: '/stateCities/'+city,
                success: function (data) {
                    if(data.success){
                        Object.keys(data.cities).forEach(function (key) {
                            var state = '<option value="'+data.cities[key]["name"]+'">'+data.cities[key]["name"]+'</option>';
                            $("#city").empty();
                            $("#city").append(state);
                            console.log(data.cities[key]["name"]);
                        });
                    }
                }
            });
                ...
                success: function (data) {
                    if(data.success){
                        Object.keys(data.states).forEach(function (key) {
                            var state = '<option data-cid="'+data.states[key]["id"]+'" value="'+data.states[key]["name"]+'">'+data.states[key]["name"]+'</option>';
                            $("#state").append(state);
                            console.log(data.states[key]["name"]);
                        });
                    }
                }
            });
